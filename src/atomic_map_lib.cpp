...
std::vector<int> get_atomic_numbers(const std::vector<std::string>& sybmols) {
	...
	for (const auto& symbol : sybmols) {
		atomic_numbers_vector.push_back(atomic_numbers[symbol]);
	}

	return atomic_numbers_vector;
}

std::set<int> get_unique_atomic_numbers(const std::vector<int>& atomic_numbers) {
	std::set<int> unique_atomic_numbers;
	for (const auto& atomic_number : atomic_numbers) {
		unique_atomic_numbers.insert(atomic_number);
	}
